...
class Ejson(
    ...
    fun decrypt(secrets: File, ignoreErrors: Boolean = false): MutableMap<String, Any> {
        if(!secrets.exists()) {
            project.logger.log(LogLevel.INFO, "Ejson: Didn't find a secrets file at $secrets...ignoring")
            return HashMap(0)
        }
        ...
        return if(execResult.exitValue != 0) {
            ...
            if(ignoreErrors) {
                ...
                throw RuntimeException("ejson exited with a non-zero value (${execResult.exitValue}) - $err")
            }
        } else {
            stdout
                .toString()
